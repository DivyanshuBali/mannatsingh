	...
	return (
		<>
			<section className={styles.listContainer}>
				<ol>
					{logs.map((log) => (
						<Link key={log.code} href={`/logs_v2/${log.id}`}>
							<li onMouseEnter={() => handleMouseEnter(log)} onMouseLeave={handleMouseLeave}>
								{log.title}
							</li>
						</Link>
					))}
			...
			<section className={styles.imageContainer}>
				{hoveredLog && <Image src={hoveredLog.bannerImage} alt={hoveredLog.title} fill className={styles.logImage} />}
			</section>

			<section className={styles.mobileCards}>
				{logs.map((log) => (
					<Link key={log.code} href={`/logs_v2/${log.id}`} className={styles.mobileCard}>
